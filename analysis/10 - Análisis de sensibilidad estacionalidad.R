packages = c('readxl', 'mapSpain', 'tsModel', 'grid', 'tsibble', 'plot3D', 'splines', 'MASS', 'tidyverse',  'splines', 'gnm', 'dlnm', 'mixmeta', 'furrr', 'lubridate', 'ggplot2', 'gridExtra', 'mgcv', 'forecast', 'tseries')
...
# Defino la componente estacional

datos.serie <- datos.serie %>%
  mutate(estación = ifelse(mm == 6|mm == 7|mm == 8, "Verano", 
                           ifelse(mm == 1|mm == 2|mm == 3|mm == 4|mm == 5,  "Antes de verano", "Después de verano")))

datos.serie$estación <- factor(datos.serie$estación,
                               levels = c("Antes de verano", "Verano", "Después de verano"))

# Represento la componente estacional de la serie

datos.serie2 <- datos.serie %>%
  group_by(estación, yy) %>%
  summarise(Total = sum(Total)) %>%
  ungroup()

datos.serie2 <- datos.serie2 %>%
  arrange(yy)

serie <- ts(datos.serie2$Total, start = 1987, frequency = 3)
...
ggseasonplot(
  ...
  main = "Gráfico de estacionalidad de la seriede nº de suicidios",
  season.labels = c("Antes de verano", "Verano", "Después de verano")
) +
  theme_bw()
...
datos.serie$stratum <- as.factor(as.factor(datos.serie$yy):as.factor(datos.serie$estación))
...
# save(model.gnm2, cbvacaj, cbTj, file = "./data/modelo.sensibilidadverano.gnm.RData")
...
load("./data/modelo.sensibilidadverano.gnm.RData") 
...
# save(Sall.gnm, file = "./data/ICDiaGnmSensibilidadVerano.RData")
load("./data/ICDiaGnmSensibilidadVerano.RData")
