...
################################################################
# DEFINICIÓN DE LA VARIABLE DE VACACIONES

# Defino la variable "periodo vacacional" que también usaré como 
# variable independiente, además del día de la semana y de la 
# temperaura media
################################################################
...
# La categoría de "Otras vacaciones" incluye todos los días puntuales considerados festivos en Asturias, es decir: el Día de Reyes, el Día del Trabajador, el Día de Asturias, el Día de la Hispanidad, el Día de Todos los Santos, el Día de la Constitución y el Día de la Inmaculada Concepción.

# Las vacaciones de verano se definieron del 15 de julio al 15 de agosto.
...
idno <- ((nproof$funx == 1 &
            nproof$dfx > 3) |
           (nproof$funx == 2 &
              nproof$dfx <= 3) | (nproof$nlag < 3 & nproof$lagnk > 1)
)
...
################################################################
# SELECCIÓN DEL MODELO NO CONDICIONAL (GLM + DLNM)

# Selecciono el modelo que mejor se ajusta a los datos 
# usando la función glm y empleando el uso de crossbasis,
# por lo tanto un modelo de retardos distribuidos no condicional.
################################################################ 
...
#   )
#   
#   dfseas <- 1 # por ausencia de estacionalidad
#   ny <- length(unique(datos.serie$yy))
#   modelj <- try(glm(
#     Total ~ cbTj + cbvacaj + dow + ns(fecha, df = ny * dfseas),
#     data = datos.serie,
#     quasipoisson,
#     na.action = "na.exclude",
#     maxit = 100
#   ))
#   cat(length(modelj$coefficients))
#   QAICj <- try(QAICM(modelj, "dev"))
#   lres <- list(
#     fit = modelj,
#     QAIC = QAICj,
#     cbT = cbTj,
...
# # Rescate del modelo
# 
# model.glm2 <- map_depth(best, 1, function(x) {
#   return(x$fit)
# })
# 
# model.glm2 <- model.glm2$proofs93 # proofs93 ha sido seleccionado como mejor modelo
# cbvacaj <- best$proofs93$cbvaca
# cbTj <- best$proofs93$cbT
# save(model.glm2, cbvacaj, cbTj, file = "./data/modelo.global.RData") # Guardo el modelo final

# El modelo final resulta tener las siguientes características:
...
yall <- coef(model.glm2)[25:30] # Guardo los coeficientes correspondientes

# Sall <- confint(model.glm2) # Calculo y guardo los intervalos de confianza de los coeficientes
# save(Sall, file = "./data/ICDiaGlm.RDAta")
load("./data/ICDiaGlm.RData")
Sall2 <- Sall[25:30, 1:2] # Selecciono sólo los intervalos relacionados con el día de la semana

eyall <- exp(yall) # Convierto los coeficientes en Riesgos Relativos
esall <- exp(Sall2) # Obtengo los intervalos de confianza para el riesgo relativo

# Represento el efecto
...
vpredcen <- crosspred(cbvacaj, model.glm2, at = 1:6, cen = 1)
...
pred <- crosspred(cbTj, model.glm2, at = tpred)                 

# Estimo la temperatura de mínima predicción de mortalidad (TMM) y su intervalo de confianza del 95%
...
predcen <- crosspred(cbTj, model.glm2, at = tpred, cen = mmt)
...
# Represento el efecto de la temperatura media
...
predcenP95 <- crosspred(cbTj, model.glm2, at = calor, cen = mmt)
...
# Miro el efecto en el punto de frío extremo (percentil 0.05)

frio <- quantile(datos.serie$temp.pob.sum,
                 probs = c(0.005),
                 na.rm = T)

predcenP1 <- crosspred(cbTj, model.glm2, at = frio, cen = mmt)
...
c(predcenP1$allRRlow, predcenP1$allRRfit, predcenP1$allRRhigh)
...
################################################################
# VALIDACIÓN DEL MODELO GLM + DLNM FINALMENTE SELECCIONADO
...
res <- residuals(model.glm2)
pacf(as.numeric(na.omit(res)), lag.max = 4, main = "Función de Autocorrelación Parcial")
...
################################################################
# SELECCIÓN DEL MODELO CONDICIONAL (GNM + DLNM)

# Selecciono el modelo que mejor se ajusta a los datos 
# usando la función gnm y empleando el uso de crossbasis,
# por lo tanto un modelo de retardos distribuidos condicional.
################################################################ 

# Defino el estrato (necesario para ajustar un modelo gnm) para el control de la tendencia. No considero la estacionalidad ya que el anterior análisis exploratorio indicó ausencia de estacionalidad en la serie de suicidios.
...
# cbvacaj <- best$proofs93$cbvaca
# cbTj <- best$proofs93$cbT
# save(model.gnm2, cbvacaj, cbTj, file = "./data/modelo.noestac.gnm.RData")

################################################################
# EXPLORACIÓN DEL MODELO GNM + DLNM FINALMENTE SELECCIONADO

# Estudio el efecto de las variables consideradas obtenido 
# mediante el modelo finalmente seleccionado
################################################################ 

load("./data/modelo.noestac.gnm.RData") 
...
# save(Sall.gnm, file = "data/ICDiaGnmNoestac.RData")
load("./data/ICDiaGnmNoestac.RData")
...
# Represento el efecto de las vacaciones
...
# Predicción de efectos respecto de MMT
...
# Efecto en el punto de calor extremo
...
################################################################
# VALIDACIÓN DEL MODELO GNM + DLNM FINALMENTE SELECCIONADO
