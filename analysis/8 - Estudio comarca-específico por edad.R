...
# Cargo la serie temporal según la comarca tanto para los individuos en edad laboral como para los mayores de 65 años
...
# de 16-65 años, así podré obtener el efecto combinado para los 
# individuos de eda edad, además de las BLUPS asociadas a los 
# de 16-65 años de cada comarca
################################################################

# Creo una lista donde cada elemento es la serie de cada comarca

ld <- split(datos.serie.1665, datos.serie.1665$comarca_DE)
names(ld) <- unique(datos.serie.1665$comarca_DE)
...
system.time({
  for(i in seq(ld)) {
    ...
    mfirst <- gnm(Total ~ cbTj+cbvacaj+dow, data=datos, family=quasipoisson, eliminate = factor(stratum), na.action = "na.exclude", subset = ind > 0)
...
#save(loweric, upperic, file="./data/ICDiaComarcaGnmEdad1665.RData")
load("./data/ICDiaComarcaGnmEdad1665.RData")
...
# Realizo un estudio comarca-específico considerando sólo a los
# >65 años, así podré obtener el efecto combinado para los >65 años
# además de las BLUPS asociadas a los >65 años de cada comarca
################################################################

# Creo una lista donde cada elemento es la serie de cada comarca

ld <- split(datos.serie.65mas, datos.serie.65mas$comarca_DE)
names(ld) <- unique(datos.serie.65mas$comarca_DE)
...
#save(loweric, upperic, file="data/ICDiaComarcaGnmEdad65mas.RData")
load("data/ICDiaComarcaGnmEdad65mas.RData")
