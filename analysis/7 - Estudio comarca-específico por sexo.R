...
# Cargo la serie temporal según la comarca tanto para hombres como para mujeres

source("./data/funciones.R")
load("./data/serie.grupos.RData")

################################################################
# HOMBRES

# Realizo un estudio comarca-específico considerando sólo a los
# hombres, así podré obtener el efecto combinado para los hombres
# además de las BLUPS asociadas a los hombres de cada comarca
################################################################

# Creo una lista donde cada elemento es la serie de cada comarca

ld <- split(datos.serie.h, datos.serie.h$comarca_DE)
names(ld) <- unique(datos.serie.h$comarca_DE)
...
# Repreentación de la curva sin corregir y de la BLUP
...
# Representación del efecto combinado
...
#save(loweric, upperic, file="./data/ICDiaComarcaGnmSexo.RData")
load("./data/ICDiaComarcaGnmSexo.RData")
...
# Realizo un estudio comarca-específico considerando sólo a las
# mujeres, así podré obtener el efecto combinado para las mujeres
# además de las BLUPS asociadas a las mujeres de cada comarca
################################################################

# Creo una lista donde cada elemento es la serie de cada comarca

ld <- split(datos.serie.m, datos.serie.m$comarca_DE)
names(ld) <- unique(datos.serie.m$comarca_DE)
...
# Representación de la curva sin ajustar y de la BLUP
...
ggplot(pov, aes(x = lag, y = RR, color = grupo)) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  #theme(povanel.grid.minor = element_blank()) +
  labs(
    x = "\nLag",
    y = "RR\n",
    colour = "",
...
#save(loweric, upperic, file="./data/ICDiaComarcaGnmSexo2.RData")
load("./data/ICDiaComarcaGnmSexo2.RData")
...
ggplot(p.dia, aes(
  ...
  geom_errorbar(aes(ymin = LowRR, ymax = HighRR, width = 0.2),
                colour = "purple",
                size = 0.8) +
  #geom_line(colour="purple3",size=0.8)+
  geom_point(colour = "purple4", size = 3) +
  geom_hline(yintercept = 1,
             size = 0.5,
             linetype = "dashed") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  #t029eme(panel.grid.minor = element_blank()) +
  labs(x = "\nDía de la semana", y = "RR\n", title = "") +
  scale_color_manual(values = colors) +
  guides(colour = guide_legend(override.aes = list(shape = NA)))
